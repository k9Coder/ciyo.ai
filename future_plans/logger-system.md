...
This task has no new tests — the integration is covered by Task 2's plugin tests and manual smoke test.
...
**Register plugin just after `void app.register(cors)` (line 27):**
```typescript
void app.register(requestLoggingPlugin)
```

**Replace the error handler body (currently line 65-68):**
```typescript
// before
...
The full updated `buildApp` function should look like this:
...
```
Expected: `{"ok":true}` response AND two JSON log lines in the server terminal:
```json
{"time":"...","level":"info","msg":"Request Started","method":"GET","url":"/health",...}
{"time":"...","level":"info","msg":"Request Completed","method":"GET","url":"/health","statusCode":200,...}
```
...
import { logger } from './logger/index.js'
import { datadogTransport } from './logger/transports/datadog.js'
...
No changes to the Logger class or any router needed.
