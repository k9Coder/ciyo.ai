...
export async function destinationGroupsRouter(fastify: FastifyInstance): Promise<void> {
  fastify.get('/destination-groups', { preHandler: requireAdminTokenOrClerkAdmin }, async (req) => {
  ...
  fastify.delete('/destination-groups/:id', { preHandler: requireAdminTokenOrClerkAdmin }, async (req, reply) => {
