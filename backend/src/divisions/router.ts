...
export async function divisionsRouter(fastify: FastifyInstance): Promise<void> {
  fastify.get('/divisions', { preHandler: requireAdminTokenOrClerkAdmin }, async (req) => {
    return listDivisions(req.tenant.id)
  })

  fastify.post('/divisions', { preHandler: requireAdminTokenOrClerkAdmin }, async (req, reply) => {
    const body = req.body as { name: string; slug: string }
    return reply.status(201).send(await createDivision(req.tenant.id, body))
  })

  fastify.patch('/divisions/:id', { preHandler: requireAdminTokenOrClerkAdmin }, async (req, reply) => {
  ...
  fastify.delete('/divisions/:id', { preHandler: requireAdminTokenOrClerkAdmin }, async (req, reply) => {
