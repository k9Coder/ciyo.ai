 ...
 * client/server split, and pretzel-desktop/qa-bridge's cli.mjs. Usage:
...
async function ensureServer() {
  ...
  throw new Error('[qa-bridge] Server failed to start within 20s')
}

// The server is a long-running background process with its own cwd
// (qa-bridge/) — relative output paths must be resolved against the
// CALLER's cwd here before sending. See pretzel-desktop/qa-bridge/cli.mjs
// for the bug this fixes (found by actually running /qa-desktop).
function resolvePathArgs(cmd, args) {
  ...
  if (cmd === 'cookie-import' && resolved[0]) {
    resolved[0] = path.resolve(process.cwd(), resolved[0])
  }
  return resolved
}
