...
PolicyDoc
  ...
  subjects[]
    ...
    name
    rules[]
      id, kind, keywords, pattern, destinations, action, message, reportLevel
...
A valid backend document produces an engine policy with an empty `baseline` and all backend rules in `custom`. Therefore, once a backend policy is cached, it replaces rather than supplements the built-in default baseline.
...
- `destinations` is validated and stored but ignored by the bridge.
