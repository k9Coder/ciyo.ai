  ...
  return (
    <div className="px-6 py-24">
      ...
      <div className="mx-auto max-w-2xl">
        <p className="mb-3 text-[11px] font-bold uppercase tracking-widest text-brand">Blog</p>
        <h1 className="mb-12 text-4xl font-extrabold tracking-tight text-ink">AI DLP Research &amp; Security Guides</h1>
        {posts.map(post => (
          <Link key={post.slug} href={`/blog/${post.slug}`}
            className="mb-6 block rounded-2xl border border-line bg-surface p-7 transition hover:border-brand">
            {post.tag && <span className="mb-2 inline-block rounded-full bg-brand-soft px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider text-brand">{post.tag}</span>}
            <h2 className="mb-2 text-[18px] font-bold text-ink">{post.title}</h2>
            <p className="mb-3 text-[13px] text-muted">{post.description}</p>
            <time className="text-[11px] text-muted">{format(new Date(post.date), 'MMMM d, yyyy')}</time>
          </Link>
        ))}
      </div>
    </div>
