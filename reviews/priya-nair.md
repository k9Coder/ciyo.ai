...
**Date:** 2026-06-08
**Scope:** ciyo-web marketing site — 22 files reviewed through brand, messaging, SEO, and accuracy lenses.

---

#### `ciyo-web/app/page.tsx` — Homepage route assembly
- [x] Reviewed
  **Verdict:** WARN
...
#### `ciyo-web/app/product/page.tsx` — Product detail page (three surfaces)
- [x] Reviewed
  **Verdict:** ISSUE
  **Findings:**
...
#### `ciyo-web/app/pricing/page.tsx` — Full pricing page with toggle
- [x] Reviewed
  **Verdict:** ISSUE
  **Findings:**
  ...
  4. **"Talk to Sales" links to `mailto:sales@ciyo.ai`.** For enterprise prospects this is fine, but there is no Calendly or demo-booking link — high friction for a buyer whose CISO instinct is to book a call, not compose an email.
...
#### `ciyo-web/app/solutions/page.tsx` — Solutions hub (by industry)
- [x] Reviewed
  **Verdict:** WARN
  **Findings:**
...
#### `ciyo-web/app/solutions/[industry]/page.tsx` — Individual industry solution pages
- [x] Reviewed
  **Verdict:** ISSUE
  **Findings:**
  1. **Stat citations missing or unverifiable.**
     ...
     - Legal: "67% of Am Law 200 firms have no AI usage policy as of 2026" — no source. If this is ciyo.ai research, say so. If it is from a third party, cite it.
     - Engineering: "1 in 3 developer AI prompts contain at least one credential or secret (ciyo.ai data, 2026)" — this one attributes ciyo.ai, which is good, but requires the underlying data to actually exist and be defensible. If this stat is based on internal platform data, it needs a methodology note or it is misleading.
...
- [x] Reviewed
  ...
  **Findings:**
  ...
  3. **No bug bounty program link.** The disclosure policy says "email security@ciyo.ai" but there is no mention of HackerOne, Bugcrowd, or a public policy page. CISOs at 500+ seat companies will want a formal program.
...
- [x] Reviewed
  ...
  **Findings:**
  ...
  4. **Meta description** — "Insights on AI security, data loss prevention, and enterprise AI governance" is decent but could include the brand name: "The Pretzel by ciyo.ai blog: ..."
  **Proposed changes:**
...
#### `ciyo-web/app/about/page.tsx` — About / company page
- [x] Reviewed
  **Verdict:** WARN
  **Findings:**
  ...
  4. **"hello@ciyo.ai — we read every email and reply to most of them"** — charming copy for a consumer brand; slightly informal for a CISO-targeted enterprise product. Consider splitting into "hello@ciyo.ai for general questions" and "sales@ciyo.ai for enterprise discussions."
  5. **`description: 'The story behind ciyo.ai and Pretzel.'`** — too vague for SEO.
  **Proposed changes:**
...
#### `ciyo-web/app/layout.tsx` — Root layout with global metadata
- [x] Reviewed
  **Verdict:** WARN
  **Findings:**
  ...
  3. **`twitter: { creator: '@ciyo_ai' }`** — confirm this Twitter/X handle exists and is active. If the account is dormant or does not exist, this tag is noise.
  ...
  **Proposed changes:**
  ...
  - Confirm the `@ciyo_ai` Twitter handle.
  - Add `alternates: { canonical: 'https://ciyo.ai' }` at the layout level and override per-page.

---

#### `ciyo-web/app/sitemap.ts` — XML sitemap generator
- [x] Reviewed
  **Verdict:** ISSUE
  **Findings:**
...
#### `ciyo-web/app/robots.ts` — Robots directive
- [x] Reviewed
  **Verdict:** PASS
...
#### `ciyo-web/components/sections/FeatureGrid.tsx` — Six-feature grid
- [x] Reviewed
  **Verdict:** PASS
...
#### `ciyo-web/components/sections/CTABanner.tsx` — Bottom-of-page CTA banner
- [x] Reviewed
  **Verdict:** WARN
  **Findings:**
  ...
  2. **"Talk to Sales" links to `mailto:hello@ciyo.ai?subject=Enterprise enquiry`** — the mailto goes to `hello@ciyo.ai`, not `sales@ciyo.ai`. The Pricing page uses `sales@ciyo.ai`. There is an inconsistency in the sales contact email across the site. Pick one and make it consistent.
  ...
  **Proposed changes:**
  - Standardise all "Talk to Sales" mailto links to `sales@ciyo.ai` with subject "Enterprise inquiry".
  - Soften the "immediately" claim.

---

#### `ciyo-web/components/sections/HowItWorks.tsx` — Three-step explainer
- [x] Reviewed
  **Verdict:** PASS
...
#### `ciyo-web/components/sections/PricingPreview.tsx` — Homepage pricing teaser
- [x] Reviewed
  **Verdict:** WARN
  **Findings:**
  ...
  3. **Hardcoded `https://app.ciyo.ai/onboarding` URLs** — these bypass the `APP_URL` config used elsewhere. If `APP_URL` ever changes (e.g., staging override), these links will not follow.
  **Proposed changes:**
  - Standardise Solo description: "For individuals validating Pretzel before recommending it to their team."
  - Replace hardcoded `https://app.ciyo.ai/onboarding` with `${APP_URL}/onboarding` imports.
...
#### `ciyo-web/components/sections/VideoDemo.tsx` — Video demo section
- [x] Reviewed
  **Verdict:** ISSUE
  **Findings:**
...
#### `ciyo-web/components/layout/Header.tsx` — Site-wide navigation header
- [x] Reviewed
  **Verdict:** WARN
  **Findings:**
  1. **No "Docs" link in the main nav** — the Footer includes a full Docs section with links to `docs.ciyo.ai`, but the header nav omits it. Enterprise evaluators frequently want to jump directly to API docs or integration guides during evaluation.
  ...
  **Proposed changes:**
  - Add "Docs" to the NAV array, linking to `https://docs.ciyo.ai`.
  - Add active route highlighting using `usePathname()`.

---

#### `ciyo-web/components/layout/Footer.tsx` — Site-wide footer
- [x] Reviewed
  **Verdict:** ISSUE
  **Findings:**
  ...
  4. **External docs links (`https://docs.ciyo.ai`, `https://docs.ciyo.ai/api`, `https://docs.ciyo.ai/enterprise`)** — these need to be verified as live. If docs.ciyo.ai does not resolve, these are dead links in every visitor's footer.
  5. **No social media links** (Twitter/X, LinkedIn) in the footer despite `twitter: { creator: '@ciyo_ai' }` in layout metadata.
  **Proposed changes:**
  ...
  - Verify docs.ciyo.ai is live before shipping.
  - Add Twitter/LinkedIn icons to the footer.

---

#### `ciyo-web/lib/config.ts` — APP_URL configuration
- [x] Reviewed
  **Verdict:** PASS
  **Findings:** Correct pattern — `NEXT_PUBLIC_APP_URL` with a sensible production default. However, `PricingPreview.tsx` hardcodes `https://app.ciyo.ai/onboarding` rather than using this export, creating an inconsistency (flagged in PricingPreview review above). Config itself is clean.
  **Proposed changes:** N/A (the fix belongs in PricingPreview.tsx).

---

#### `ciyo-web/lib/posts.ts` — Blog post data store
- [x] Reviewed
  **Verdict:** ISSUE
  **Findings:**
  ...
  5. **The single post description reads "We analysed 100,000 AI prompts"** — if this data does not come from the Pretzel platform (i.e., ciyo.ai does not yet have 100,000 scanned prompts in production), this claim is inaccurate and should be corrected or the post should not be published yet.
  **Proposed changes:**
