...
| **mykka-web** | Vercel Git integration | Push to `master` (auto) |
...
| `VITE_API_BASE_PROD` | Production backend URL (e.g. `https://api.mykka.ai`) | Your custom domain or Render service URL |
...
| `CORS_ORIGIN` | `https://console.mykka.ai` | Or comma-separated: `https://console.mykka.ai,https://mykka.ai`. Must match pretzel-console domain. |
...
| `SMTP_USER` | e.g. `postmaster@mg.mykka.ai` | SMTP provider username |
| `SMTP_PASS` | `...` | SMTP provider password/API key |
| `ADMIN_BASE_URL` | `https://console.mykka.ai` | Used to build invite links sent to members |
...
| `SMTP_FROM` | `noreply@mykka.ai` | You want a different sender address |
| `STRIPE_SUCCESS_URL` | `https://mykka.ai/welcome` | You build a custom post-checkout page |
| `STRIPE_CANCEL_URL` | `https://mykka.ai/pricing` | You build a custom cancel page |
| `PAYPAL_RETURN_URL` | `https://mykka.ai/welcome` | Custom post-subscription page |
| `PAYPAL_CANCEL_URL` | `https://mykka.ai/pricing` | Custom cancel page |
...
## Part 4 — Vercel Environment Variables (mykka-web)

Set these at: **Vercel dashboard → mykka-web project → Settings → Environment Variables**

| Var | Value | Notes |
|---|---|---|
| `NEXT_PUBLIC_API_BASE` | `https://api.mykka.ai` | Production backend URL |
...
4. Fill in store listing if first upload:
   ...
   - Privacy policy URL: `https://mykka.ai/privacy`
...
| Item | Where | Why |
|---|---|---|
| Email templates | Clerk → Customization → Emails | Replace default "Clerk" branding with mykka.ai / Pretzel branding |
| Allowed domains | Clerk → Settings → Restrictions | Optional: restrict sign-up to company email domains for early beta |
| Webhook endpoint | Clerk → Webhooks | Must point to `https://api.mykka.ai/webhooks/clerk` and have `user.created`, `user.updated`, `organization.created` events enabled |
...
| Item | Where | Why |
|---|---|---|
| Webhook endpoint | Stripe → Developers → Webhooks | Must point to `https://api.mykka.ai/billing/stripe/webhook`. Events: `checkout.session.completed`, `customer.subscription.updated`, `customer.subscription.deleted` |
...
| Webhook | PayPal → Developer → Webhooks | Point to `https://api.mykka.ai/billing/paypal/webhook`. Events: `BILLING.SUBSCRIPTION.ACTIVATED`, `BILLING.SUBSCRIPTION.CANCELLED` |
...
[ ] Check Vercel deploy succeeds (mykka-web)
[ ] Hit https://api.mykka.ai/health → expect { "ok": true }
[ ] Open console.mykka.ai → sign in → dashboard loads
[ ] Open mykka.ai → marketing site loads
