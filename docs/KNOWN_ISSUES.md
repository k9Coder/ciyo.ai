...
| Medium | Console Docker | Implementation | Nginx CSP `script-src`/`connect-src`/`frame-src` still don't allowlist Clerk's domains, so Clerk-backed auth breaks in the containerized console even though the port mismatch and local-backend `connect-src` block are now fixed (`nginx.conf.template` + `API_ORIGIN`, see `docker-compose.yml`). | Sign-in/auth flows fail in the Docker full-stack console; non-auth pages and local-API calls work. | Chloe Dubois / Ryan Kowalski |
